...
- GitHub 上原提交 aaac7fe 的 CI 已成功（运行 35125855370），不只是在已清空的本地沙盒通过。
- 新增 Docker 内部 Host 白名单回归；Companion 测试现在为 8 项。
- 独立打包后，解压到没有 node_modules 的目录，清除 NODE_PATH / NODE_OPTIONS：启动、鉴权、网页资源、MCP 工具、Prompt 预览、禁用付费生成和服务重启恢复通过。
- 独立包不包含实际 NAI 密钥、运行数据库或用户素材；真实 NAI / 宿主 / VPS 验证的状态仍保持“未验证”。
