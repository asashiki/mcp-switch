...
export const characterSchema = z
  .object({
    ...
    persona: tags.default(""),
    characterPrompt: tags.min(1),
    stylePrompt: tags.default(""),
    negativePrompt: tags.default(""),
    characterNegativePrompt: tags.default(""),
    defaultExpression: id.default("neutral"),
    defaultScene: id,
    expressions: z.record(
      id,
      z.object({ prompt: tags, asset: asset.optional() }),
    ),
    scenes: z.record(
      id,
      z.object({
    ...
    generation: z
      .object({
        ...
        steps: z.number().int().min(1).max(28).default(28),
        scale: z.number().min(0).max(10).default(5),
        sampler: z
          .enum(["k_euler_ancestral", "k_euler", "k_dpmpp_2m", "k_dpmpp_sde"])
          .default("k_euler_ancestral"),
        noiseSchedule: z
          .enum(["karras", "native", "exponential"])
          .default("karras"),
      })
      .default({
  ...
  .superRefine((c, ctx) => {
    for (const [field, collection] of [
      ["defaultExpression", "expressions"],
      ["defaultScene", "scenes"],
      ["defaultOutfit", "outfits"],
    ] as const) {
      if (!Object.hasOwn(c[collection], c[field]))
        ctx.addIssue({
          code: "custom",
          message: `${field} must exist in ${collection}`,
          path: [field],
        });
    }
    if (c.generation.width * c.generation.height > 1_048_576)
      ctx.addIssue({
        code: "custom",
        message: "Maximum one megapixel in v0.1",
        path: ["generation"],
      });
  });
...
export function loadConfig(env = process.env) {
  ...
  return {
    ...
    charactersFile: resolve(
      env.COMPANION_CHARACTERS || "./examples/characters.json",
    ),
    dailyLimit: z.coerce
      .number()
      .int()
      .min(0)
      .max(100)
      .parse(env.COMPANION_DAILY_LIMIT ?? "10"),
