...
export function createHttp(service: Companion) {
  ...
  const mcpHandler = createMcpHandler(() => createServer(service), {
    legacy: "stateless",
    responseMode: "auto",
  });
  ...
  app.addHook("onRequest", async (req, reply) => {
    ...
    const publicMedia =
      req.url.startsWith("/media/") || req.url.startsWith("/jobs/");
    ...
    if (
      (req.url.startsWith("/api/") || req.url.split("?")[0] === "/mcp") &&
      cfg.token
    ) {
  ...
  app.route({
    ...
    handler: async (req, reply) => {
      await handler(req.raw, reply.raw, req.body);
      return reply;
    },
  });
  app.get("/admin", async (req, reply) =>
    reply
      .type("text/html")
      .send(
        await readFile(new URL("../dist/admin.html", import.meta.url), "utf8"),
      ),
  );
  app.get("/stage", async (req, reply) =>
    reply
      .type("text/html")
      .send(
        await readFile(new URL("../dist/stage.html", import.meta.url), "utf8"),
      ),
  );
  app.get("/api/config", async () => ({
    characters: service.getCharacters(),
    generationEnabled: cfg.generationEnabled,
    providerConfigured: !!cfg.naiToken,
    dailyLimit: cfg.dailyLimit,
  }));
  app.put("/api/config", async (req) => {
    service.saveCharacters(req.body);
    return { ok: true };
  });
  app.post("/api/preview", async (req) => {
    ...
    if (
      !Object.hasOwn(c.expressions, a.expression) ||
      !Object.hasOwn(c.scenes, a.sceneId) ||
      !Object.hasOwn(c.outfits, a.outfit)
    )
      throw new Error("Unknown scene selection");
    const { compilePrompt } = await import("./novelai.js");
    return {
      scene: service.view(s),
      preview: compilePrompt(s, a.composition, 123456789),
    };
  });
  ...
  app.post("/api/assets", async (req) => {
    ...
    if (!Buffer.isBuffer(bytes))
      throw new Error("Upload raw PNG, JPEG or WebP bytes");
    const png = bytes
      .subarray(0, 8)
    ...
    await writeFile(join(cfg.dataDir, "assets", name), bytes, { mode: 0o600 });
    return { asset: name, url: service.asset(name) };
  });
  app.get<{
    Params: { name: string };
    Querystring: { expires?: string; sig?: string };
  }>("/media/:name", async (req, reply) => {
    const name = z
      .string()
      .regex(/^[a-zA-Z0-9_-]+\.(png|jpg|webp)$/)
      .parse(req.params.name);
    if (!service.verify(`/media/${name}`, req.query.expires, req.query.sig))
      return reply
        .code(403)
        .send({ error: "Media link expired or invalid; restore the scene" });
    try {
      return reply
        .type(
          name.endsWith(".png")
        ...
        .send(await readFile(join(cfg.dataDir, "assets", name)));
    } catch {
  ...
  app.get<{
    ...
    if (!service.verify(`/jobs/${id}`, req.query.expires, req.query.sig))
      return reply.code(403).send({ error: "Job link expired or invalid" });
