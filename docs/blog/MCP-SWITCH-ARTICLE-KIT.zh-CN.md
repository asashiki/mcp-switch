...
这份资料对应 2026 年 9 月整理后的 `mcp-switch`。它把已经完成的代码、仍需实机确认的部分、官方协议变化和截图顺序放在一起。写文章时不必全部照搬。
...
1. **旧客户端不用一起升级。** 同一个 `/mcp` 端点兼容 2026-07-28 与旧版客户端，上游也自动协商。
2. **一次调用不再反复重连。** 长生命周期上游 runtime、single-flight 初始化、catalog TTL 与 idle shutdown，避免一个 stdio 工具调用连续拉起多个子进程。
...
5. **OAuth 和容器边界更严格。** resource/audience 绑定、PKCE、refresh replay 吊销、Host/Origin allowlist、rootless/read-only 容器都已实现。
6. **App Lab 把“加载失败”拆成能检查的问题。** 它检查 resource link、MIME、bridge、CSP、URI namespace 和 output schema；预览需要人手动打开，不调用真实工具。
7. **WebMCP 让 AI 与人共享控制台。** 控制台静默注册五个顶层 Site tools；只读动作可直接执行，页面动作只改变当前视图，配置动作只生成待审草稿。
...
已经完成的证据：Switch 连接 `music-mcp` 0.2 的真实 HTTP 服务后，发现 3 个工具和播放器资源，并在 App Lab 中把两个 UI 工具判断为 standards-first，诊断通过。
...
2. **新版 VPS 控制台总览**：四个 KPI 与系统健康。图注：「网关升级没有把控制台改造成另一个聊天客户端。」
3. **Music MCP App Lab**：展开组件诊断和播放器预览。图注：「先判断问题发生在 resource、MIME、bridge 还是宿主增强，不盲改 CSS。」
4. **ChatGPT 中的五个 Site tools**：图注：「工具属于当前控制台页面，离开页面后自动失效。」
5. **AI 生成接入草稿**：名称和 URL 已填，Token/OAuth 仍为空，保存按钮等待人工点击。图注：「AI 可以准备，人决定是否真的连接。」
6. **GitHub Actions 与容器健康检查**：图注：「自动测试之外，候选版本还在旧版旁边跑了一次真实部署。」
...
- 自动化覆盖 9 个 WebMCP 契约测试和 33 个 gateway/auth/store/remote/App Lab 测试，共 42 个。
- GitHub Actions 会独立执行依赖安装、Compose 配置检查、类型检查、测试与生产构建。
- 新版采用独立 Compose project、端口与 SQLite volume 部署，旧版不被覆盖。
...
- [PR #4：协议保真网关](https://github.com/asashiki/mcp-switch/pull/4)
- [PR #6：WebMCP 控制面与最终整理](https://github.com/asashiki/mcp-switch/pull/6)
