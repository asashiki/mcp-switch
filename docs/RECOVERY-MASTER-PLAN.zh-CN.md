...
## 3.2 WebMCP 控制面（独立审查分支）

- 控制台登录后在顶层页面注册 5 个 WebMCP Site tools；不支持的浏览器保持原行为；
- 智能体可读取脱敏网关总览、列出/检查上游，并在用户可见页面打开 App Lab；
- 配置能力采用 proposal → review → commit：智能体只能填 HTTP(S) 待审草稿，不能保存、连接、
  授权、删除服务器，也不能接触 Token、Secret、Header 或 env；
- 上游派生结果标为不可信内容，列表不返回 endpoint、错误文本和可能泄露环境结构的字段；
- 工具注册绑定 AbortSignal，页面卸载/登出自动清理，API fetch 同步支持取消；
- 新增独立契约测试、产品设计说明与挑战赛三分钟演示提纲。
